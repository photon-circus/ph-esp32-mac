...
use super::framework::{DMA_BASE, EMAC, IO_MUX_BASE, TestContext, TestResult};
    ...
    for (name, offset, is_input) in pins {
        ...
        if ok {
            info!(
                "  {} MCU_SEL=5 {}",
                name,
                if is_input { "FUN_IE=1" } else { "" }
            );
        } else {
            error!(
                "  {} MCU_SEL={} FUN_IE={} EXPECTED MCU_SEL=5",
                name, mcu_sel, fun_ie
            );
            all_ok = false;
        }
...
pub fn test_dma_descriptor_chain() -> TestResult {
    ...
    info!(
        "  RX desc base=0x{:04X}_{:04X}, TX desc base=0x{:04X}_{:04X}",
        (rx_base >> 16) & 0xFFFF,
        rx_base & 0xFFFF,
        (tx_base >> 16) & 0xFFFF,
        tx_base & 0xFFFF
    ...
    for i in 0..4u32 {
        ...
        if rdes3 != expected_next || !dma_owns {
            error!(
                "  Desc[{}] NEXT=0x{:04X}_{:04X} expected 0x{:04X}_{:04X}, OWN={}",
                i,
                (rdes3 >> 16) & 0xFFFF,
                rdes3 & 0xFFFF,
                (expected_next >> 16) & 0xFFFF,
                expected_next & 0xFFFF,
